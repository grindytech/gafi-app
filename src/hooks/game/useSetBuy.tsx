...
export default function useSetBuy(
  onSuccess: () => void,
  ...
  const mutation = useMutation(
    async (params: {
      ...
      const txExecute = api?.tx.game.setBuy(params.package, params.unitPrice);
      return txExecute?.signAndSend(account, options || {}, txCallback);
    },
    {
      mutationKey: 'game-set-buy',
      onError: (error: Error) => {
        toast({
          position: 'top-right',
  ...
  const setBuy = (
    nftsPackage: GafiSupportGameTypesPackage,
    unitPrice: string
  ) => {
    refetchLoad(true);

    mutation.mutate({ package: nftsPackage, unitPrice });
  };
  return { isLoading, setBuy };
}
