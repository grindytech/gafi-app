...
export default function useBuyBundle(
  onSuccess: () => void,
  ...
  const mutation = useMutation(
    async (params: { trade: number; bid: string }) => {
      ...
      const txExecute = api?.tx.game.buyBundle(params.trade, params.bid);
      return txExecute?.signAndSend(account, options || {}, txCallback);
    },
    {
      mutationKey: 'game-bid-bundle',
      onError: (error: Error) => {
        toast({
          position: 'top-right',
  ...
  const bidBundle = (trade: number, bid: string) => {
    refetchLoad(true);

    mutation.mutate({ trade, bid });
  };
  return { isLoading, bidBundle };
}
