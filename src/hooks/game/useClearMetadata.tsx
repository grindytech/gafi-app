...
export default function useClearMetadata(
  onSuccess: () => void,
  ...
  const mutation = useMutation(
    async (params: { collection: number; item: number }) => {
      ...
      const txExecute = api?.tx.game.clearMetadata(
        params.collection,
        params.item
      );
    ...
    {
      mutationKey: 'game-clear-metadata',
      onError: (error: Error) => {
        toast({
          position: 'top-right',
  ...
  const clearMetadata = (collection: number, item: number) => {
    refetchLoad(true);

    mutation.mutate({ collection, item });
  };
  return { isLoading, clearMetadata };
}
