...
export default function useSetAcceptAdding(
  onSuccess: () => void,
  ...
  const mutation = useMutation(
    async (params: { game: number; collection: number }) => {
      ...
      const txExecute = api?.tx.game.setAcceptAdding(
        params.game,
        params.collection
      );
    ...
    {
      mutationKey: 'game-set-accept-adding',
      onError: (error: Error) => {
        toast({
          position: 'top-right',
  ...
  const setAcceptAdding = (game: number, collection: number) => {
    refetchLoad(true);

    mutation.mutate({ game, collection });
  };
  return { isLoading, setAcceptAdding };
}
