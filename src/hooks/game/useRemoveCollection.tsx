import { NumberInputStepperProps, useToast } from '@chakra-ui/react';
...
export default function useRemoveCollection(
  ...
  const mutation = useMutation(
    async (params: { game: number; collection: number }) => {
      ...
      const txExecute = api?.tx.game.removeCollection(
        params.game,
        params.collection
      );
    ...
    {
      mutationKey: 'game-remove-collection',
      onError: (error: Error) => {
        toast({
          position: 'top-right',
  ...
  const removeCollection = (game: number, collection: number) => {
    refetchLoad(true);

    mutation.mutate({ game, collection });
  };
  return { isLoading, removeCollection };
}
