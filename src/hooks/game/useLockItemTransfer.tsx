...
export default function useLockItemTransfer(
  onSuccess: () => void,
  ...
  const mutation = useMutation(
    async (params: { collection: number; item: number }) => {
      ...
      const txExecute = api?.tx.game.lockItemTransfer(
        params.collection,
        params.item
      );
    ...
    {
      mutationKey: 'game-lock-item-transfer',
      onError: (error: Error) => {
        toast({
          position: 'top-right',
  ...
  const lockItemTransfer = (collection: number, item: number) => {
    refetchLoad(true);

    mutation.mutate({ collection, item });
  };
  return { isLoading, lockItemTransfer };
}
